...
const activeChannels = {}; // { channel: { emotes:[], users:{}, client:tmi.Client } }
...
// 🔹 Get Twitch user ID (for tracking)
  ...
  client.on("message", async (ch, tags, msg, self) => {
    ...
    if (used.length > 0) {
      for (const e of used) {
        ...
        users[username].emoteUsage[e.name] = (users[username].emoteUsage[e.name] || 0) + 1;
      }
